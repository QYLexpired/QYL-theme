...
######  · 自定义属性`css`的值中各样式需用英文分号隔开，比如需要设置一个块具有红色边框和蓝色背景，则`css`的值可填入：border:1px solid red;background-color:blue，详细方法可见[CSS教程](https://www.runoob.com/css/css-tutorial.html)

###### · 以下列表为本主题的自定义属性，块标/文档处点击右键-属性-添加自定义属性，即可设置该块/该文档的效果

|自定义属性|适用类型|属性值|功能|
| ------------| -------------------| -----------------------------------------------------------------------------------------------| -------------------------------------|
|​`css`​|任意块|​`css代码`​|自定义该块的css样式|
