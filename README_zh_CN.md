...
* light配色：经典、夕阳、森林、海洋、糖果、薰衣草、云雾、霜禾、回忆、湖畔、香雪兰、象牙、珊瑚、薄荷、青柠、灰雀

* dark配色：经典、勃艮第、玄青、墨翠、灰幕、赤霞、苔雪、暮霭、乌木、午夜、沧溟、蒸汽
...
* **专注编辑模式**：使当前编辑的块自动保持在编辑器的垂直中心（手机端不生效），且模糊未编辑的块来突出当前编辑的块，不能与聚焦块高亮提示同时开启（此功能需消耗性能，若感知到卡顿请及时关闭）
...
* **毛玻璃效果**：使全局菜单、窗口应用毛玻璃风格（渲染样式需消耗性能，若感知到卡顿请及时关闭）
* **主题动画**：添加大量灵动、活泼的动画效果（渲染样式需消耗性能，若感知到卡顿请及时关闭）
...
* [Rem Craft主题](https://github.com/svchord/Rem-Craft)：象牙配色、乌木配色、极简化风格
* [JeffreyChen](https://ld246.com/member/JeffreyChen)：图片九宫格排列实现方案的优化
...
* 增加QYL自定义属性图片样式选项：`九宫格排列`，设置后，块内的图片按照九宫格排列，最多显示9张，鼠标悬停在该块上则显示全部图片
* 致谢：`九宫格排列`的完美实现离不开[J佬](https://ld246.com/member/JeffreyChen)帮助优化！

##### v3.0.8

* 增加文档块`line-height`自定义属性选项，可用于调整文档的文字行间距
* 增加超级块`sb-colgap`自定义属性选项，可用于调整水平排列超级块的列间距
* 优化部分性能
* 优化部分`style`自定义属性样式
* 修复开启毛玻璃样式后，菜单选中项背景异常的问题
* 修复开启毛玻璃样式后，菜单分隔线消失的问题

##### v3.0.7

* 修复通过快捷键呼出块菜单时，QYL自定义属性项无法出现的问题
* 扁平化风格不再绑定部分毛玻璃样式，但可以与毛玻璃样式同时开启
* 优化部分毛玻璃样式
